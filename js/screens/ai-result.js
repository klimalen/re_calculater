...
const TIPS = [
  'Рядом с блюдом на фото размести столовый прибор или руку — точность распознавания веса будет заметно выше',
  'Сфотографируй информацию о КБЖУ с упаковки продукта, чтобы взять данные оттуда',
  'Сложное блюдо? Заполни комментарий к фото, чтобы у AI была подсказка',
  'После распознавания можно поправить граммовку — это пропорционально изменит КБЖУ',
  'В текстовом режиме можно перечислить сразу всё, что может не влезть на фотографию',
  'Приложение запоминает твой прогресс — загляни в статистику через неделю',
  'Самый частый скрытый источник калорий — соусы и заправки, про них легко забыть',
  'Белок насыщает дольше всего: он задерживает опустошение желудка и снижает аппетит',
  'В 100 г огурца — 15 ккал. Овощи почти не нужно считать',
  'Цели по питанию можно добавить через Профиль. Статистика питания станет более наполненной',
  'Голод и жажда легко спутать — стакан воды перед едой помогает не переесть',
  'Жареное в масле блюдо может быть на 30–50% калорийнее запечённого аналога',
  'Клетчатка замедляет усвоение углеводов — цельнозерновой хлеб насыщает дольше белого',
];

function _shuffleTips() {
  const a = [...TIPS];
